...
% MODIFIED BY MATE GARAI (garaimp0@sewanee.edu or mate.garai@colorado.edu April, 2024)
%
...
if ischar(key)
  switch double(key)
    ...
    case 115
        ...
        name = inputdlg("Save file as:");
        if ~isempty(name)
            if ~isempty(name{1})
                writematrix([x',ry'],strcat(name{1},"_FFT.txt"));
            else
    ...
    case 107
        ...
        disp('Save output as .txt file......S')
        disp(' ')     
      case 113
      ...
      otherwise  % if key pressed is unnasigned
       disp('Press K to print out list of keyboard commands') % print unassigned key code
   end % switch
end % if
